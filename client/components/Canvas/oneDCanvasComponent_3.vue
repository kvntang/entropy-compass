...
const createImageDoc = async (
  ...
  try {
    ...
    console.log(
      `ImageDoc created successfully! Coordinate: ${coordinate}, Type: ${type}, Step: ${step}, PromptIndex: ${promptIndex}, RefactoredStep: ${refactoredstep}`
    );
  ...
  if (canvasContainer.value) {
    const sketch = new p5((p) => {
      ...
      p.setup = async () => {
        ...
        // Initialize the first ImageDoc if staticPositions is empty
      ...
      p.draw = () => {
        ...
        if (isDragging) {
          ...
          if (
            ...
            p.text(
              promptSteps,
              lastImage.pos.x + gridSize / 2,
              lastImage.pos.y + (gridSize + padding) * promptSteps + gridSize / 2
            );
            ...
            p.fill(255);
            p.text(
              lastImage.promptIndex,
              newX + gridSize / 2,
              lastImage.pos.y + gridSize / 2
            );
          }
        }
      ...
      p.mouseReleased = async () => {
        ...
        if (isDragging) {
          ...
          if (
            ...
            let promptSteps = Math.floor(
              (mouseYWorld - (lastImage.pos.y + gridSize / 2)) / (gridSize + padding)
            );
      ...
      p.doubleClicked = () => {
        ...
        for (let i = 0; i < staticPositions.length; i++) {
          ...
          if (
            mouseXWorld >= img.pos.x &&
            mouseXWorld <= img.pos.x + gridSize &&
            mouseYWorld >= img.currentY &&
            mouseYWorld <= img.currentY + gridSize
          ) {
